...
<style>
    ...
    .supp-action button {display: block;padding-top: 3px !important;padding-bottom: 3px !important;}
    ...
    .left-names div {padding-bottom: 1px;}
                ...
                <div class="row">
                    ...
                    <div class="col-md-12 mt-3">
                        <fieldset style="border: 1px #ddd solid">
                            <legend class="px-1" style="border: 1px #ddd solid;margin-left: 1em;width:70px;font-size: 12px;">2024</legend>
                                ...
                                <div class="col-8">
                                    <div class="left-names" style="display: inline-block;text-align: right;">
                                        ...
                                        <div style="display: none;">
                                            <button class="btn close-supplier-year">Close year</button>
                                        </div>
                                    </div>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="title" id="largeModalLabel"><?php echo $_GET["new-supplier"]; ?> <small style="font-size:12px"> - {{$data['store']->name}}</small></h4>
            </div>
            <div class="modal-body"> 
	            <form id="basic-form" class="new-supplier">
	                @csrf
					<input type="hidden" name="shopstore" value="store"> 
					<input type="hidden" name="sid" value="{{$data['store']->id}}"> 
	                <div class="row clearfix">
...
<script>
    ...
    $(function () { 
        ...
        // checkPurchasesCart("<?php echo $data['store']->id.'~'.$data['supplier']->id; ?>");
        supplierYearSummary("<?php echo $data['store']->id.'~'.$data['supplier']->id; ?>");
    });
    ...
    $(document).on('change', '.change-supplier', function(e) {
        ...
        supplierYearSummary("<?php echo $data['store']->id; ?>~"+sid);
    });
    ...
    function getSupplierDetails(sid) {
        ...
        $.get("/suppliers/get-supplier-details/"+sid, function(data){
            ...
            checkPurchasesCart("<?php echo $data['store']->id; ?>~"+sid);     
        });
    }

    function checkPurchasesCart(ssid) {
        $.get("/suppliers/supplier-purchases-cart/"+ssid+"~store", function(data){
    ...
    function supplierYearSummary(ssid) {
        $.get("/suppliers/supplier-year-summary/"+ssid+"~store", function(data){
    ...
    function getSupplierPurchasesDepositsDetails(ssid) {
        ...
        $.get("/suppliers/get-purchases-deposits/"+ssid+"~store", function(data){
    ...
    $(document).on('click', '.supplier-purchase-btn', function(e){
        ...
        checkPurchasesCart("<?php echo $data['store']->id; ?>~"+suppid);
        $('.supp-tq').text(0);
        $('.supp-tp').text(0);
    ...
    $(document).on('click', '.clear-purchased-cart', function(e){
        ...
        $('.purchase-products-form button').prop('disabled',true);
        $('.add-loader').css('display','inline');
        $.get("/suppliers/clear-purchased-cart/<?php echo $data['store']->id.'~'.$data['supplier']->id.'~store'; ?>", function(data){
            $('.purchase-products-form button').prop('disabled',false);
            $('.add-loader').css('display','none');
            if(data.status == 'success') {
    ...
    $(document).on('click', '.submit-purchased-cart', function(e){
        ...
        $.get("/suppliers/submit-purchased-cart/<?php echo $data['store']->id.'~'.$data['supplier']->id.'~store'; ?>", function(data){
            $('.purchase-products-form button').prop('disabled',false);
            $('.add-loader').css('display','none');
            if(data.status == 'success') {
    ...
    $(document).on('submit', '.edit-submitted-details-form', function(e){
        ...
        var suppid = (new URL(location.href)).searchParams.get('supplier_id');
        var storeid = "<?php echo $data['store']->id; ?>";
        
        $.ajaxSetup({ headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') } });
        var formdata = new FormData(this);
        formdata.append('storeid',storeid);
        formdata.append('suppid',suppid);
        formdata.append('status','update purchased items store');
        $.ajax({
            type: 'POST',
            url: '/submit-data',
            processData: false,
    ...
    $(document).on('click', '.delete-purchased-item', function(e){
        ...
        $.get("/suppliers/clear-purchased-row-2/"+rowid+"~store", function(data){
            if(data.status == 'success') {
    ...
    $(document).on('click', '.close-supplier-year', function(e){
        e.preventDefault();
        $.get("/suppliers/supplier-close-year-balance/<?php echo $data['store']->id.'~'.$data['supplier']->id; ?>", function(data){
