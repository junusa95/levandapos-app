        ...
        data: formdata,
            success: function(data) {
                ...
                if (data.success) {
                    ...
                    popNotification('warning','Error! Customer not created.');
                }
            }
    });
});
...
$(document).on('submit', '.new-shop', function(e){
    ...
    if (name.trim() == null || name.trim() == '' || cashier.trim() == null || cashier.trim() == '') {
...
$(document).on('click','.searched-item',function(e){
    ...
    var price = $(this).attr('container');
    var check = $(this).attr('check');
    var pname = $(this).attr('pname');
    var avaQty = $(this).attr('qty');
    ...
    if (check == 'sales') {
        ...
        var existingRow = $('.items-in-cart tr.sr-' + pid + ', #cart-selected tr.sr-' + pid);

        if (existingRow.length > 0) {
            var qtyInput = existingRow.find('.mo-quantity');
            var currentQty = parseFloat(qtyInput.val()) || 0;
            qtyInput.val(currentQty + 1);
            calculateQuantity(pid);
        } else {
            var html = `
            <tr class="sr-${pid}">
                <td>
                    <div class="row">
                        <div class="col-12 ra-name mb-1 text-wrap" style="padding-left:5px">
                            ${pname}<span class="aq ml-1 px-1"></span>
                        </div>
                        ...
                        <div class="col-12">
                            <div class="row">
                                ...
                                <div class="col-12 d-flex justify-content-end">
                                    <input type="hidden" class="form-control product_id" name="product_id" value="${pid}" rid="${pid}">
                                    <span><input type="number" class="form-control quantity mo-quantity" id="quantity-${pid}" avaQty="${Number(avaQty)}" price="${Number(price)}" name="quantity" value="1" rid="${pid}" onkeyup="calculateQuantity(${pid})"></span>
                                    <span><i class="fa fa-times mt-2 mx-1"></i></span>
                                    <span>
                                        <input type="number" class="form-control sprice mo-price" id="amount-${pid}" name="sprice" value="${Number(price)}" rid="${pid}" onkeyup="calculateQuantity(${pid})" ${disabled}>
                                    </span>
                                    <div class="mt-1 ml-1" style="font-size: 15px">
                                      = <b id="total-${pid}" class="totalAmount">${Number(price).toLocaleString("en")}</b>
                                    </div>
                                    <div class="" style="float:right;margin-top:-20px">
                                     <span class="remove-sr" val="${pid}"><i class="fa fa-times"></i></span>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        //         console.log(data.sale[0]['squantity']);
        ...
        //         $('.totalQ').html(parseFloat(data.sale[0]['squantity']));
        //         $('.totalP').html(Number(data.sale[0]['sprice']).toLocaleString("en"));     
        //         return;
        //     }
        //     if (data.error) {
        //         popNotification('warning',data.error);
        ...
        //         +'<div class="col-12 r-name mb-1">'+data.data.pname+'<span class="aq"></span></div>'
...
function calculateTotalCatsValue(){
    ...
    $('#cart-selected .totalAmount').each(function() {
    ...
    $('.totalQ').html(parseFloat(totalQty));
    $('.totalP').html(Number(totalPrice).toLocaleString("en"));

     $('[name="paidamount"]').val(totalPrice);
    $('.amounttopay').html(Number(totalPrice).toLocaleString("en"));
    $('.s-change').html(0);

    var cartData = document.getElementsByClassName("items-in-cart");
    localStorage.setItem('shop_id', shop_id);
    localStorage.setItem('items-in-cart', cartData[0].innerHTML);
    localStorage.setItem('totalQ', parseFloat(totalQty));
    localStorage.setItem('totalP', Number(totalPrice).toLocaleString("en"));
}

function calculateQuantity(pid){
    var quantity = $('#quantity-'+pid).val();
    var price = $('#amount-'+pid).val();

    newAmount = price * quantity;

    if (!isNaN(newAmount)) {
        $('#amount-'+pid).val(price);
        $('#total-'+pid).html(Number(newAmount).toLocaleString('en'))
    }else{
        $('#amount-'+pid).val(0);
        $('#total-'+pid).html(0)
    }
